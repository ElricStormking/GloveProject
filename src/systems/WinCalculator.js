...
window.WinCalculator = class WinCalculator {
    ...
    calculateMatchWin(match, betAmount) {
        ...
        if (!symbolInfo || symbolInfo.payout === 0) {
            return 0;
        }
        
        // Base win calculation
        const baseWin = symbolInfo.payout * betAmount;
        
        // Calculate match size multiplier (more symbols = higher multiplier)
        const matchSizeMultiplier = this.calculateMatchSizeMultiplier(match.length);
        
        // Get highest symbol multiplier in the match
        const symbolMultiplier = this.getHighestSymbolMultiplier(match);
        
        // Calculate total win
        const totalWin = baseWin * matchSizeMultiplier * symbolMultiplier;
    ...
    calculateMatchSizeMultiplier(matchSize) {
        // Progressive multiplier based on match size
        const baseMatches = window.GameConfig.MIN_MATCH_COUNT;
        const extraMatches = matchSize - baseMatches;
        
        if (extraMatches <= 0) return 1;
        
        // Each extra match adds 0.5x multiplier
        return 1 + (extraMatches * 0.5);
    }
